...
namespace MLAgents
{
    ...
    public class RayPerception2D : RayPerception
    {
        ...
        public override List<float> Perceive(
            ...
            float startOffset=0.0f, float endOffset=0.0f)
        {
            ...
            {
                m_EndPosition = startingTransform.TransformDirection(
                    PolarToCartesian(rayDistance, angle));
                if (Application.isEditor)
                {
                    Debug.DrawRay(startingTransform.position,
                        m_EndPosition, Color.black, 0.01f, true);
                }

                var subList = new float[detectableObjects.Length + 2];
                m_Hit = Physics2D.CircleCast(startingTransform.position, 0.5f, m_EndPosition, rayDistance);
                if (m_Hit)
                {
